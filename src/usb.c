...
__code uint8_t USB_HID_RAW_REPORT_DESCR[] = {
    ...
    0x95, 63,          //   Report Count (63) - 1 byte for ID, 63 for data to make 64 byte packet?
                       //   Actually, if Report ID is used, it takes 1 byte.
                       //   If EP size is 64, we can fit ID + 63 bytes.
                       //   Let's set Report Count to 63 so total is 64.
    0x81, 0x00,        //   Input
